...
SELECTEDMETA1_ANY_OF_SCHEMAS = ["HtmlSelectedMeta", "SelectedMetaWithUploadId"]

class SelectedMeta1(BaseModel):
    """
    ...
    # data type: SelectedMetaWithUploadId
    anyof_schema_1_validator: Optional[SelectedMetaWithUploadId] = None
    # data type: HtmlSelectedMeta
    anyof_schema_2_validator: Optional[HtmlSelectedMeta] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[HtmlSelectedMeta, SelectedMetaWithUploadId]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: List[str] = Literal[SELECTEDMETA1_ANY_OF_SCHEMAS]
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = SelectedMeta1.model_construct()
        error_messages = []
        # validate data type: SelectedMetaWithUploadId
        if not isinstance(v, SelectedMetaWithUploadId):
            error_messages.append(f"Error! Input type `{type(v)}` is not `SelectedMetaWithUploadId`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in SelectedMeta1 with anyOf schemas: HtmlSelectedMeta, SelectedMetaWithUploadId. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[SelectedMetaWithUploadId] = None
        try:
            instance.actual_instance = SelectedMetaWithUploadId.from_json(json_str)
            return instance
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into SelectedMeta1 with anyOf schemas: HtmlSelectedMeta, SelectedMetaWithUploadId. Details: " + ", ".join(error_messages))
        else:
            return instance
