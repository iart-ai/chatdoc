...
from chatdoc.models.upload_package_type import UploadPackageType
...
PACKAGETYPE_ANY_OF_SCHEMAS = ["UploadPackageType", "str"]
...
class PackageType(BaseModel):
    ...
    # data type: UploadPackageType
    anyof_schema_1_validator: Optional[UploadPackageType] = None
    # data type: str
    anyof_schema_2_validator: Optional[StrictStr] = None
    if TYPE_CHECKING:
        actual_instance: Optional[Union[UploadPackageType, str]] = None
    else:
        actual_instance: Any = None
    any_of_schemas: List[str] = Literal[PACKAGETYPE_ANY_OF_SCHEMAS]
    ...
    def actual_instance_must_validate_anyof(cls, v):
        instance = PackageType.model_construct()
        error_messages = []
        # validate data type: UploadPackageType
        if not isinstance(v, UploadPackageType):
            error_messages.append(f"Error! Input type `{type(v)}` is not `UploadPackageType`")
        else:
            return v
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when setting the actual_instance in PackageType with anyOf schemas: UploadPackageType, str. Details: " + ", ".join(error_messages))
        else:
            return v
    ...
    def from_json(cls, json_str: str) -> Self:
        ...
        # anyof_schema_1_validator: Optional[UploadPackageType] = None
        try:
            instance.actual_instance = UploadPackageType.from_json(json_str)
            return instance
        except (ValidationError, ValueError) as e:
             error_messages.append(str(e))
        ...
        if error_messages:
            # no match
            raise ValueError("No match found when deserializing the JSON string into PackageType with anyOf schemas: UploadPackageType, str. Details: " + ", ".join(error_messages))
        else:
            return instance
